...
class Duwi {
    ...
    {
        $this->_LOAD = & get_instance();
        //$this->_LOAD->load->model('Modeldb','Mdb');
    }
  ...
  function folderSize ($dir)
	...
	public function menu($levelakses){
		...
		$menu_akhir=array();
		$menu=$this->_LOAD->Mdb->read($main_menu)->result();
		if(count($menu)>0){
			foreach ($menu as $index => $row) {
				...
				$submenu=$this->_LOAD->Mdb->read($sub_menu)->result();
				if(count($submenu)>0){
